#!/bin/bash
# Phase 0-8 Re-test - Verify Phases 0-8 still work after Phase 9
#
...
echo "============================================================"
echo "Phase 0-8 Re-test - Verify Previous Phases Still Work"
echo "============================================================"
echo
...
echo
echo "Testing Phase 9 (Policy Evaluation)..."
echo

# Test eval_policy script exists
EVAL_SCRIPT="/root/colcon_ws/src/ackermann-vehicle-gzsim-ros2/ackermann_drl/scripts/eval_policy.py"
if [ -f "$EVAL_SCRIPT" ]; then
    echo "✓ eval_policy.py exists"
    if [ -x "$EVAL_SCRIPT" ]; then
        echo "✓ eval_policy.py is executable"
    else
        echo "⚠ eval_policy.py is not executable"
    fi
else
    echo "✗ eval_policy.py not found"
    exit 1
fi

# Test eval model load
if python3 /root/colcon_ws/src/ackermann-vehicle-gzsim-ros2/test/test_eval_model_load.py > /dev/null 2>&1; then
    echo "✓ Eval model load test passed"
else
    echo "⚠ Eval model load test failed (check manually)"
fi

# Test evaluation functions exist
if python3 -c "
import sys
...
with open(script_path, 'r') as f:
    content = f.read()
assert 'def evaluate_policy' in content, 'evaluate_policy function not found'
assert 'def print_statistics' in content, 'print_statistics function not found'
assert 'success_rate' in content, 'success_rate calculation not found'
assert 'battery_levels' in content, 'battery tracking not found'
assert 'offroad_violations' in content, 'offroad violation tracking not found'
print('✓ Evaluation functions present')
" 2>/dev/null; then
    echo "✓ Evaluation functions present"
else
    echo "✗ Evaluation functions missing"
    exit 1
fi

echo
echo "============================================================"
echo "Phase 0-8 Re-test: All checks passed!"
echo "============================================================"
echo
echo "Previous phases remain valid after Phase 9 changes."
